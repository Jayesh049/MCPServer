  ...
  return (
    <div className="detail">
      <div className="panel">
        ...
        {disease.inputSpec.kind === "image" ? (
          <ImageUploader
            acceptedMimeTypes={disease.inputSpec.acceptedMimeTypes}
            onChange={setImage}
          />
        ) : (
      ...
      <div style={{ display: "grid", gap: 14 }}>
        {result ? (
          ...
          <div className="panel">
            <h2>Result</h2>
            <p className="subtle">
              Provide input on the left and click <strong>Run detection</strong>. The
              pipeline will run detection → resolution → solution. When a known disease
              is detected, a care plan with exercises, top doctors + medications, and
              positive affirmations will appear below.
            </p>
          </div>
        )}
        ...
        {result && !carePlan && (
          <div className="panel">
            <h2>Care Plan</h2>
            <p className="subtle">
              No care plan was auto-attached for this prediction (the result looks
              normal/low-risk). You can still load the synthetic care plan for{" "}
              <strong>{disease.name}</strong> on demand.
            </p>
